...
def get_msgs_df_info(df):
    ...
    replies_count_dict = dict(
        Counter([u for r in df.replies if r != None for u in r]))
...
def get_messages_from_channel(channel_path):
    ...
    channel_msgs = [json.load(open(channel_path + "/" + f))
                    for f in channel_json_files]

    df = pd.concat([pd.DataFrame(get_messages_dict(msgs))
                   for msgs in channel_msgs])
...
def get_tagged_users(data):
    """get all @ in the messages"""
    return data['msg_content'].map(lambda x: re.findall(r'@U\w+', x))
